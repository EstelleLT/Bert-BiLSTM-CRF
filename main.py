...
class Trainer:
    ...
    def test(self):
        self.model.load_state_dict(torch.load(os.path.join(self.output_dir, "pytorch_model_ner.bin")),strict=False)
        self.model.eval()
        preds = []
        trues = []
...
def main(data_name):
    ...
    # with open(os.path.join(args.data_path, "train.jsonl"), "r", encoding='utf-8') as fp:
    #     train_data = fp.read().split("\n")
    # train_data = [json.loads(d) for d in train_data]
    train_data = []
    with open(os.path.join(args.data_path, "train.jsonl"), "r", encoding='utf-8') as f:
        for line in f:
            data = json.loads(line)
            train_data.append(data)

    # with open(os.path.join(args.data_path, "dev.jsonl"), "r", encoding='utf-8') as fp:
    #     dev_data = fp.read().split("\n")
    # dev_data = [json.loads(d) for d in dev_data]
    dev_data = []
    with open(os.path.join(args.data_path, "dev.jsonl"), "r", encoding='utf-8') as f:
        for line in f:
            data = json.loads(line)
